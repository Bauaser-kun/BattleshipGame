...
public class Board extends Parent {
    ...
    public class Cell extends Rectangle {
            ...
            if (ship != null) {
                ...
                if (!ship.isNotSinked()) {
                    ...
                    for (int i = 0; i < ship.size; i++){
                        if (isValidPoint(this.columns + i, this.rows)) {
                            Cell checkVerticalCoordinateUp = getCell(this.columns + i, this.rows);
                            sink(checkVerticalCoordinateUp);
                        }

                        if (isValidPoint(this.columns - i, this.rows)) {
                            Cell checkVerticalCoordinateDown = getCell(this.columns - i, this.rows);
                            sink(checkVerticalCoordinateDown);
                        }

                        if (isValidPoint(this.columns, this.rows + i)) {
                            Cell checkHorizontalCoordinateUp = getCell(this.columns, this.rows + i);
                            sink(checkHorizontalCoordinateUp);
                        }

                        if (isValidPoint(this.columns, this.rows - i)) {
                            Cell checkHorizontalCoordinateDown = getCell(this.columns, this.rows - i);
                            sink(checkHorizontalCoordinateDown);
                        };
                    }
                }
                return true;
        ...
        private void sink(Cell sinked) {
            if (sinked.ship != null && sinked.wasAimed == true) {
                sinked.setFill(Color.RED);
            }
        }
